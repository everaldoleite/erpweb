...
import br.com.erpweb.persistence.entities.Categoria;
import br.com.erpweb.view.util.JsfUtil;
import br.com.erpweb.view.util.PaginationHelper;
import br.com.erpweb.session.bean.CategoriaFacade;
...
public class CategoriaController implements Serializable {
    ...
    @EJB
    private br.com.erpweb.session.bean.CategoriaFacade ejbFacade;
    ...
    public String prepareCreate() {
        current = new Categoria();
        selectedItemIndex = -1;
        return "erp_create_categoria";
    }

    public String create() {
        try {
            getFacade().create(current);
            JsfUtil.addSuccessMessage("Categoria criada com sucesso !");
    ...
    public String prepareEdit() {
        current = (Categoria) getItems().getRowData();
        selectedItemIndex = pagination.getPageFirstItem() + getItems().getRowIndex();
        return "erp_edit_categoria";
    }

    public String update() {
        try {
            getFacade().edit(current);
            JsfUtil.addSuccessMessage("Categoria atualizada com sucesso !");
            return "erp_list_categoria";
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/Bundle").getString("PersistenceErrorOccured"));
            return null;
